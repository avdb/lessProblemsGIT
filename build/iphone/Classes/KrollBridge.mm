...
NSString * lessProblemsGIT$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation lessProblemsGITObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_lessproblemsgit);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
-(void)injectPatches
{
	// called to inject any lessProblemsGIT patches in JS before a context is loaded... nice for 
...
{
	// create lessProblemsGIT global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_lessproblemsgit = [[lessProblemsGITObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_lessproblemsgit];
	
	NSString *_lessproblemsgitNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _lessproblemsgitNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_lessproblemsgit valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:lessProblemsGIT$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.lessproblemsgit.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.lessproblemsgit.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
